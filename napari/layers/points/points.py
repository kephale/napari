...
class _BasePoints(Layer):
    ...
    def __init__(
        ...
        *,
        ...
        if ndim is None:
            ndim = self._points_data.shape[1]
        ...
        state.update(
            {
                ...
                if (hasattr(self.data, "size") and self.data.size)
    ...
    def _view_data(self) -> np.ndarray:
        ...
        if len(self._indices_view) > 0:
            data = self._points_data[np.ix_(self._indices_view, self._slice_input.displayed)]
        else:
...
class Points(_BasePoints):
    ...
    # FIXME @jookuma
    #  - is there a better approach than copying the documentation from _BasePoints?
